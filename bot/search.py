"""Tavily web search for booth-photo pipeline (company research)."""
...
async def _search_one_company(
    ...
    try:
        response = await client.search(
            ...
            max_results=5, include_answer="basic",
        )
        ...
        return CompanyResearch(
            name=company.name, booth_context=company.booth_context,
            website=website, description=description[:500],
        ...
        return CompanyResearch(
            name=company.name, booth_context=company.booth_context, search_failed=True,
        )
...
async def _search_founder_background(
    ...
    query = (
        f"{company.name} founder CEO origin education university"
...
async def research_companies(
    ...
    tasks = [_search_one_company(client, c) for c in companies]
    results = await asyncio.gather(*tasks, return_exceptions=True)

    researched = []
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            logger.error("Research failed for %s: %s", companies[i].name, result)
            researched.append(CompanyResearch(
                name=companies[i].name, booth_context=companies[i].booth_context,
                search_failed=True,
            ))
        else:
            researched.append(result)

    # Phase 2: Leadership + founder background
    ...
    for i in range(n):
        if isinstance(all_results[i], str):
            researched[i].leadership_info = all_results[i]
        if isinstance(all_results[n + i], str):
            researched[i].founder_background = all_results[n + i]

    logger.info(
        "Researched %d companies (%d failures)",
        len(researched), sum(1 for r in researched if r.search_failed),
    )
    return researched
